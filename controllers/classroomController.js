...
export async function getClassrooms(request, reply) {
  try {
    ...
    const groupedClassrooms = data.reduce((acc, classroom) => {
      const { floor, id, name, schoolId, created_at, items } = classroom;
      if (!acc[floor]) {
        acc[floor] = [];
      }
      acc[floor].push({ id, name, floor, schoolId, created_at, items });
      return acc;
    }, {});
    return reply.code(200).send(JSON.stringify(groupedClassrooms));
  } catch (err) {
    return reply.code(500).send(err);
  }
}
